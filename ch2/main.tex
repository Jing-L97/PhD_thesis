...
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.12\textwidth}>{\raggedright\arraybackslash}p{0.25\textwidth}>{\raggedright\arraybackslash}p{0.25\textwidth}>{\raggedright\arraybackslash}p{0.12\textwidth}>{\raggedright\arraybackslash}p{0.198\textwidth}@{}}
...
Word form & Familiarization followed by preference for words versus part-words; spontaneous or reported vocabulary production & Probability-based word/nonword discrimination or generation of forms & Partial & \citep{saffran1996statistical,Aslin1998,nguyen2022word} \\
Word meaning & Spoken word--referent matching by looking or pointing & Image--label or image--phrase matching & Partial; close requires matched trials & \citep{bergelson_at_2012,Lany2011,Wang2025} \\
...
Syntax & Artificial-grammar preference or child production; later explicit judgments & Probability comparison for grammatical minimal pairs & Partial; potentially close for matched discrimination & \citep{Marcus1999,warstadt2020blimp,Hu2020} \\
...
Children's productive morphology is classically probed by eliciting inflected or derived forms of nonce words in meaningful sentence and picture contexts \citep{BerkoGleason1958,engelmann2019input,michaly2025development}. Perceptual work can reveal earlier sensitivity to morphological structure---for example, 6- and 8-month-olds distinguish relations between suffixed nonce forms and stems \citep{kim2021morphology}---but that outcome is not the same as productive generalization. The point of the Wug paradigm is that a response cannot be a stored form: it must extend a pattern to a novel stem. Computational evaluations range from supervised paradigm completion, where a model receives a source form and a derivational slot \citep{Cotterell2017}, to Wug-style generation in which a nonce form appears in a sentence frame \citep{weissweiler2023counting,anh2024morphology}.
...
Early comprehension is commonly assessed with looking-while-listening or forced-choice procedures: the child hears a word while viewing a target and distractor, and gaze or pointing toward the named referent indexes comprehension \citep{bergelson_at_2012,Lany2011,Bergelson2017,barbir2023rapid}. Multimodal benchmarks offer a related matching task, but their alternatives must be checked explicitly. In BabyVLM, the Visual Two-Word Test asks a model to select a matching two-word phrase for an image; the Labeled-S task instead selects an image from four candidates \citep{Wang2025}. These tasks test grounded lexical or compositional knowledge, but are not themselves matched trials with children. A closer comparison would give both learners the same referents and linguistic contrast and justify the link between model choice and child gaze or pointing.

Important differences remain. Children's referential decisions unfold in time and are supported by prosody, joint attention, and expectations about the speaker. Static image--text pairs remove those cues. Conversely, distributional models are often evaluated through lexical-relation classification or similarity among learned representations \citep{Lenci2022}; these evaluations do not directly reproduce an infant category-sorting or referent-selection task. Such representations can be explanatory predictors, but they should not be labeled behaviorally aligned without showing that they predict trial-level child responses.

\paragraph{Syntactic structure.}
Artificial grammar learning exposes infants or children to sequences generated by a hidden regularity and tests their response to novel conforming and violating sequences \citep{Marcus1999,gervain2012binding}. Child production additionally reveals generalization and characteristic errors that an endpoint judgment may conceal. Model evaluations such as BLiMP and SyntaxGym contrast grammatical and minimally different ungrammatical sentences using probability or surprisal \citep{warstadt2020blimp,Hu2020,marvin2018targeted,salazar_masked_2020}.
...
Looking and violation-of-expectation responses provide evidence about infants' expectations concerning events \citep{aslin2007s,Margoni2023}. To use such a response as evidence of sentence comprehension, the contrast must depend on the linguistic description rather than on an unexpected event alone. Event-related potentials, including N400 responses to semantic violations, provide a complementary processing measure \citep{friedrich2006early,lozano2025development}. Models are evaluated through plausibility minimal pairs or word-by-word surprisal,
\[
...
False-belief and related tasks ask whether children predict an agent's behavior from that agent's informational state rather than from reality \citep{WimmerPerner1983,Wellman2001}. Language models can answer textual versions of these scenarios and sometimes reach high accuracy \citep{kosinski2024evaluating,Strachan2024}. Surface-level task identity is deceptive here. Children act or answer within a socially interpretable event; models usually generate text after a decontextualized narrative, and the answer may be recoverable from recurring linguistic patterns. Existing tasks establish benchmark performance, not equivalence between grounded mental-state reasoning and textual continuation. The textual scenario tasks reviewed here also leave pragmatic production---adapting an utterance to a particular interlocutor's knowledge and communicative needs---largely untested.

Across the reviewed domains, matched perceptual materials and a common decision structure offer a particularly direct route to alignment, as in word--referent matching. Preference tasks and minimal-pair scores share contrasts but differ in how evidence is expressed. Similarity measures are generally explanatory proxies rather than behavioral equivalents. Production can in principle be scored jointly, but only if elicitation and opportunity are aligned. These distinctions motivate the representative evidence maps in Tables~\ref{tab:litreview:model-studies} and~\ref{tab:litreview:child-studies}.

\subsection{Selected empirical coverage}
\label{sec:litreview:selected-studies}

The following tables map selected experiments, evaluation methods, and reviews to the domains discussed above. Their purpose is to show which combinations of input and outcome are represented, and where the human and model literatures still fail to meet at the same task. An entry documents a relevant method or finding; it does not imply that the cited work implements a matched developmental comparison.

\begingroup
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.16\textwidth}>{\raggedright\arraybackslash}p{0.12\textwidth}>{\raggedright\arraybackslash}p{0.20\textwidth}>{\raggedright\arraybackslash}p{0.25\textwidth}>{\raggedright\arraybackslash}p{0.21\textwidth}@{}}
\caption[Selected computational studies of developmental language learning]{Selected computational studies and evaluation methods spanning the outcome domains reviewed in this chapter. Their relevance to developmental comparison does not imply that all were designed as models of acquisition.}\label{tab:litreview:model-studies}\\
\toprule
\textbf{Study} & \textbf{Domain} & \textbf{Model or task} & \textbf{Training or evaluation input} & \textbf{Primary outcome} \\
\midrule
\endfirsthead
\multicolumn{5}{@{}l}{\scriptsize Table~\ref{tab:litreview:model-studies} continued}\\
\toprule
\textbf{Study} & \textbf{Domain} & \textbf{Model or task} & \textbf{Training or evaluation input} & \textbf{Primary outcome} \\
\midrule
\endhead
...
\bottomrule
\endlastfoot
\citep{Schatz2013} & Phonology & ABX evaluation & Speech tokens and learned acoustic representations & Contrast discrimination error \\
\citep{Schatz2014} & Phonology & Noise-sensitive ABX & Controlled speech and acoustic degradation & Robustness of discrimination \\
\citep{Dunbar2017} & Phonology & Zero-resource speech systems & English, French, and Mandarin speech & ABX and representation quality \\
\citep{dunbar2021zero} & Phonology / lexical / syntax & ZeroSpeech evaluation suite & Untranscribed speech with lexical and syntactic probes & ABX, spoken-word, and grammar scores \\
\citep{lavechin2023babyslm} & Speech development & BabySLM & Developmentally motivated speech input & Phonetic, lexical, and syntactic scores \\
\citep{matusevych2023infant} & Phonology & Alternative acoustic learners & English, French, and Japanese speech & Contrast-specific developmental trajectories \\
\citep{poli2024modeling} & Phonology & Contrast-learning model & Speech embedded in broader ambient sound & Trajectory of discrimination \\
\citep{Cotterell2017} & Morphology & Sequence-to-sequence derivation & Source forms with derivational paradigm slots & Derived-form accuracy \\
\citep{weissweiler2023counting} & Morphology & Wug-style LM evaluation & Nonce roots in linguistic contexts & Productive inflection \\
\citep{anh2024morphology} & Morphology & Generative language models & Multilingual Wug-style items & Human-rated generalization accuracy \\
\citep{nguyen2022word} & Word form & Spoken language model probes & Continuous or segmented speech & Lexical discrimination and segmentation \\
\citep{vong2025robustness} & Word meaning & Multimodal neural learners & Child-specific SAYCam audiovisual streams & Robustness of word--referent learning across children \\
\citep{Wang2025} & Word / compositional meaning & BabyVLM evaluation framework & SAYCam and transferred image--caption data & Image--phrase and image--label matching \\
\citep{Lenci2022} & Lexical semantics & Semantic-relation classifier & Distributional representations and lexical-relation datasets & Relation-classification performance \\
\citep{warstadt2020blimp} & Syntax & BLiMP minimal pairs & Controlled grammatical contrasts & Forced-choice accuracy \\
\citep{Hu2020} & Syntax & SyntaxGym-style targeted evaluation & Sentences instantiating syntactic dependencies & Surprisal interactions and accuracy \\
\citep{marvin2018targeted} & Syntax & Targeted LM evaluation & Grammatical and ungrammatical sentence pairs & Relative sentence probability \\
\citep{salazar_masked_2020} & Syntax / semantics & Pseudo-log-likelihood scoring & Masked language models and minimal pairs & Sentence-level acceptability \\
\citep{hale2001probabilistic} & Sentence processing & Probabilistic parser & Incrementally parsed sentences & Word-by-word surprisal \\
\citep{SmithLevy2013} & Sentence processing & Probabilistic language model & Adult reading materials & Relation between surprisal and reading time \\
\citep{frank2015erp} & Sentence meaning & Recurrent language model & Sentence stimuli paired with EEG evidence & Relation between surprisal and N400 \\
\citep{kauf2023event} & Sentence meaning & Autoregressive language models & Plausible and implausible event descriptions & Semantic minimal-pair accuracy \\
\citep{michaelov2024strong} & Sentence meaning & Large language models & ERP stimulus materials & Surprisal--N400 correspondence \\
\citep{kosinski2024evaluating} & Pragmatics & Prompted language models & Textual false-belief scenarios & Mental-state answer accuracy \\
\citep{Strachan2024} & Pragmatics & Comparative LLM evaluation & Theory-of-mind tasks used with humans & Cross-task accuracy and error profile \\
\end{longtable}
\endgroup
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.17\textwidth}>{\raggedright\arraybackslash}p{0.14\textwidth}>{\raggedright\arraybackslash}p{0.20\textwidth}>{\raggedright\arraybackslash}p{0.23\textwidth}>{\raggedright\arraybackslash}p{0.20\textwidth}@{}}
\caption[Selected studies of language development]{Selected infant and child experiments, reviews, and meta-analyses illustrating the behavioral and neural measures discussed in this chapter.}\label{tab:litreview:child-studies}\\
\toprule
\textbf{Study} & \textbf{Domain / age} & \textbf{Paradigm} & \textbf{Stimulus or setting} & \textbf{Observed response} \\
\midrule
\endfirsthead
\multicolumn{5}{@{}l}{\scriptsize Table~\ref{tab:litreview:child-studies} continued}\\
\toprule
\textbf{Study} & \textbf{Domain / age} & \textbf{Paradigm} & \textbf{Stimulus or setting} & \textbf{Observed response} \\
\midrule
\endhead
...
\bottomrule
\endlastfoot
\citep{Eimas1971} & Phonology / young infants & High-amplitude sucking & Synthetic voice-onset-time contrast & Recovery of sucking response \\
\citep{Werker1984} & Phonology / 6--12 months & Conditioned head turn & Native and non-native consonant contrasts & Detection of sound change \\
\citep{Best1988} & Phonology / infancy & Habituation & Non-native click consonants & Dishabituation or looking recovery \\
\citep{Kuhl1992} & Phonology / 6 months & Conditioned response & Vowel prototypes and variants & Category-sensitive discrimination \\
\citep{Sundara2018} & Phonology / 6--10 months & Infant-controlled habituation & Language-specific coronal contrasts & Looking-time recovery \\
\citep{BerkoGleason1958} & Morphology / preschool and school age & Wug elicitation & Pictures and nonce words & Produced plural and past-tense forms \\
\citep{engelmann2019input} & Morphology / preschool children & Elicited production & Real and novel verbs in Finnish and Polish & Inflectional generalization \\
\citep{kim2021morphology} & Morphology / 6--8 months & Segmentation and recognition & Suffixed nonce forms and bare stems & Sensitivity to morphological relatedness \\
\citep{michaly2025development} & Morphology / school-age children & Comprehension and production & Real- and pseudoword derivational analogies & Accuracy and use of morphological elements \\
\citep{saffran1996statistical} & Word segmentation / 8 months & Familiarization preference & Artificial continuous speech & Word versus part-word listening \\
\citep{Aslin1998} & Word segmentation / 8 months & Head-turn preference & Artificial speech stream & Statistical segmentation preference \\
\citep{Pelucchi2009} & Word segmentation / 8 months & Familiarization preference & Natural Italian child-directed speech & Word versus part-word listening \\
\citep{grafestes2013prosody} & Word learning / 17 months & Habituation word learning & Novel labels in infant- or adult-directed prosody & Learning of label--object mappings \\
\citep{bergelson_at_2012} & Word meaning / 6--9 months & Looking while listening & Familiar nouns and paired images & Target-directed gaze \\
\citep{Lany2011} & Word meaning / second year & Forced-choice comprehension & Novel word with two referents & Gaze to selected target \\
\citep{Bergelson2017} & Word meaning / infancy & Looking while listening & Named objects in visual scenes & Time course of target gaze \\
\citep{barbir2023rapid} & Word meaning / toddlers & Forced-choice comprehension & Spoken word and paired pictures & Looking or pointing choice \\
\citep{Marcus1999} & Syntax / 7 months & Artificial grammar learning & Repetition patterns over syllables & Listening preference at test \\
\citep{gervain2012binding} & Sequence structure / newborns & Functional near-infrared spectroscopy & Speech sequences varying identity relations and position & Cortical response to identity and position \\
\citep{Margoni2023} & Expectations / infancy & Conceptual review of violation of expectation & Event contrasts across cognitive domains & Interpretation of looking and other responses \\
\citep{friedrich2006early} & Word / sentence meaning / toddlers & Picture--word semantic violation & Congruent and incongruent spoken items & N400 response \\
\citep{lozano2025development} & Semantic processing / bilingual toddlers & ERP experiment & Semantically congruent and incongruent materials & Development of N400 effects \\
\citep{WimmerPerner1983} & Pragmatics / preschool onward & False-belief task & Character, object transfer, and search story & Predicted search location \\
\citep{Wellman2001} & Pragmatics / childhood & Meta-analysis of false-belief tasks & Diverse belief scenarios & Age-related proportion correct \\
\end{longtable}
\endgroup
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{ch2/fig/measure.pdf}
  \caption[Thresholded and graded measures of competence]{Schematic contrast between thresholded and graded evaluation. A milestone analysis treats evidence for a specified competence as present once an explicit statistical criterion is met. A graded analysis retains continuous performance and supports comparison of strength and trajectory. Both depend on the validity of the underlying task. The horizontal axis is a schematic exposure-to-age mapping, not a shared natural time unit. Statistical threshold crossings also depend on sample size and measurement uncertainty; the shaded regions do not by themselves identify an acquisition boundary.}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.92\textwidth]{ch2/fig/noise.pdf}
...
Synthetic augmentation attempts to expand such corpora without returning to adult web register. Paraphrases and variation sets generated from child-directed utterances can increase lexical or syntactic diversity and improve some formal evaluations \citep{haga2024babylm}. But register imitation is not equivalent to interaction. Models generating caregiver-like dialogue can reproduce local lexical and utterance statistics while overstating alignment and failing to reproduce turn-by-turn contingency \citep{liu2024benchmarking}. Synthetic data are best described by the property they add---paraphrastic variety, simplified syntax, or a target vocabulary---rather than as globally child-like.

\paragraph{Language-only audio.}
Speech supplies information removed by transcription: segmental variation, prosody, speaker identity, timing, and cues to turn and utterance boundaries. These signals are central to phonetic category formation and segmentation \citep{Kuhl2006,saffran1996statistical}. Read-speech and audiobook corpora offer clean acoustic conditions and relatively reliable annotation, but their adult-directed register and performance style differ from home interaction. Spontaneous adult speech improves discourse naturalness without necessarily matching what is addressed to a child.
...
Egocentric video more directly samples the visual stream available to individual children. SAYCam follows a small number of infants longitudinally and has supported demonstrations of word--referent learning from a single child's visual and linguistic experience \citep{sullivan_saycam_2022,vong2024grounded}. BabyView broadens the recording effort and improves visual quality and participant diversity \citep{Long2024}. Yet a head-mounted camera is not an infant's perceptual system: camera field of view differs from gaze and attention, and usable paired speech is far scarcer than web-scale image--text examples. High ecological validity therefore comes with limited coverage and measurement uncertainty of its own.

\paragraph{Language, vision, and social interaction.}
Children learn in exchanges structured by turn taking, joint attention, communicative intention, and contingent caregiver response \citep{Tomasello1995,Csibra2009}. Dyadic corpora retain some of this structure. Shared book-reading recordings combine caregiver and infant perspectives in a constrained but genuinely interactive setting \citep{ahamat2025shared}; ECOLANG annotates caregiver speech, gesture, and gaze in interactions with preschool children \citep{gu2025ecolang}; and game-based corpora extend multimodal dialogue to middle childhood \citep{bodur_chico_2021,goumri_chica_2024}. Their value lies in temporal coordination across channels, not merely in the coexistence of audio and video.
...
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.14\textwidth}>{\raggedright\arraybackslash}p{0.18\textwidth}>{\raggedright\arraybackslash}p{0.20\textwidth}>{\raggedright\arraybackslash}p{0.24\textwidth}>{\raggedright\arraybackslash}p{0.18\textwidth}@{}}
...
\midrule
\endfirsthead
\multicolumn{5}{@{}l}{\scriptsize Table~\ref{tab:litreview:input-resources} continued}\\
\toprule
\textbf{Resource} & \textbf{Modality and register} & \textbf{Interactional structure} & \textbf{Developmental value} & \textbf{Principal limitation} \\
\midrule
\endhead
...
BabyLM \citep{warstadt2023findings} & Mixed text with child-directed and simplified components & Some dialogue after transcription & Controlled 10M/100M-word budgets & Predominantly textual; heterogeneous sources \\
...
Synthetic CDS variation sets \citep{haga2024babylm,liu2024benchmarking} & Generated child-directed-style text & Simulated turns or paraphrases & Scalable control of surface register & Limited contingency and inherited generator bias \\
...
COCO \citep{lin_microsoft_2014} & Curated images and descriptive captions & No contingent interaction & Controlled object and scene descriptions & Captioning task unlike child-directed talk \\
...
SAYCam \citep{sullivan_saycam_2022,vong2024grounded} & Infant-perspective video with speech & Naturalistic home activity & Longitudinal, child-specific grounding & Few children and camera--attention mismatch \\
...
ECOLANG \citep{gu2025ecolang} & Caregiver speech, gesture, and gaze & Semi-naturalistic object talk & Explicit cross-channel coordination & Preschool sample and limited training scale \\
CHICO/CHICA \citep{bodur_chico_2021,goumri_chica_2024} & Multimodal task dialogue & Collaborative word games & Naturalistic communicative adaptation & Task- and age-specific interaction \\
\end{longtable}
\endgroup
...
\begin{enumerate}
  ...
  \item \textbf{The exposure budget.} Give unique and total tokens or hours, epochs, sampling and curriculum, amount processed at each checkpoint, and a justified range for the human comparison. Specify datastore contents and inference-time access, and identify information supplied by pretrained components or synthetic-data generators. Distinguish resources that affect the learner from evaluators used only to measure it.
  \item \textbf{The learner and intervention.} Report architecture, tokenization, initialization or pretraining, learning objective, optimization and model selection, and the resource or signal varied relative to the baseline.
...
The review identifies three connected gaps that motivate the empirical chapters that follow. First, the reviewed lexical benchmarks leave a need for a production-oriented comparison that jointly controls model exposure, generated output quantity, and the frequency composition of the evaluated vocabulary. Chapter~\ref{chp2_chap} addresses this gap with \meCDI{}, comparing child vocabulary reports with the reliable appearance of target words in model generation across exposure conditions. This operationalizes the present chapter's argument that a vocabulary count is meaningful only when observation opportunities, lexical units, and developmental time are defined jointly.

Second, differences between child and model syntax may reflect memory and lexical exposure rather than grammatical representation alone. Chapter~\ref{chp3_chap} tests whether access to stored instances through $k$NN retrieval changes language models' sensitivity to syntactic minimal pairs and whether it reduces a lexical-frequency gap. Retrieval provides a computational test of instance access under a common outcome measure. Its frequency-stratified evaluation and analyses of retrieved information also identify when this AI method is useful; the episodic-memory interpretation rests on the functional property being tested.

Third, existing child-oriented corpora preserve linguistic register more readily than interactional contingency. Chapter~\ref{chp4_chap} addresses two distinct questions. Its dialogue benchmark evaluates how well pretrained models reproduce child--caregiver language at word, utterance, and dialogue levels. A separate learning study compares fixed reward predictors derived from natural caregiver responses and tests their effects on generated grammaticality and controlled grammatical preferences. The simulated caregivers do not supply these rewards. Together, the studies make simulation fidelity and the utility of particular feedback signals separately testable, while leaving reciprocal online learning as a further question.
...
Calibration is necessary for developmental interpretation but not sufficient for mechanistic identity. Two systems can receive similar observable input and produce similar responses while relying on different internal representations. Stronger mechanistic evidence requires intervention, process-level prediction, and generalization to new tasks, not only behavioral correlation \citep{marr_vision_1982,Perfors2011}.
...
The strongest conclusion licensed by aligned success is a conditional sufficiency claim: a specified learner can acquire a specified outcome from specified input. Mechanistic conclusions require controlled variation and process-level evidence. This logic guides the following chapters on expressive vocabulary under defined exposure, instance retrieval with shared syntactic outcomes, and the separate evaluation of dialogue simulation and caregiver-derived learning signals.
