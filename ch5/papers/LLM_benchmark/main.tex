% !TeX root = ../../../main.tex
% Body of paper at ch5/papers/LLM_benchmark/main.tex
...
Finally, both benchmarking approaches were implemented under zero-shot and few-shot conditions with two objectives: a) for the zero-shot condition, the goal is to characterize the baseline behavior of role- and age-prompted LLMs without conversational demonstrations, and b) for the few-shot condition, the goal is to measure LLMs' ability to improve (relative to the zero-shot baseline) under a combined intervention consisting of conversational examples and output-length instructions, and to test the sensitivity of the metrics to these changes.
...
To prepare the data for analysis and generation, we restructured these conversations into utterance-response pairs.  When consecutive turns came from the same speaker (which is often the case with caregivers), we decomposed these into multiple utterance-response pairs and inserted \texttt{<SILENCE>} tokens to mark the positions where the non-speaking interlocutor did not contribute. This preprocessing step preserved the sequence of recorded contributions while creating an alternating format for generation. An inserted \texttt{<SILENCE>} marks the absence of a recorded contribution at that position; it is not a measurement of pause duration or evidence of a deliberate silent response. The resulting benchmarking dataset comprised 6,600 interaction pairs containing a total of 73,300 word tokens. This corpus was further characterized by an asymmetric distribution between participant types, with 26,300 tokens produced by children and 47,000 tokens produced by caregivers.
...
Both conditions specified the interlocutor role and the child's age. The supplied few-shot templates additionally instructed caregivers to use at most 50 words and children to use at most 6 words (Section~\ref{sec:prompts}). The zero-shot/few-shot comparison therefore measures the combined effect of demonstrations and length instructions, rather than isolating either component.
...
Finally, at the level of \textbf{dialogue} dynamics, we distinguish two measures. \textit{Semantic alignment} concerns similarity between interlocutors' contributions in an exchange pair \citep{duran2019align,french2024aligning,misiek2020development}, represented using sentence embeddings \citep{reimers-2019-sentence-bert}. \textit{Semantic diversity} concerns variation among a single speaker's contributions across the conversation. Following \citet{guo2023curious}, this within-speaker measure is the average pairwise cosine distance (1 minus cosine similarity) between utterance embeddings. Higher diversity therefore indicates greater dispersion among that speaker's contributions, rather than greater alignment with the interlocutor.
% AUTHOR CHECK: confirm the exact alignment aggregation and the observation/unit used in each regression.
...
The age-conditioned child-LLMs recover several trends in the CHILDES reference, including under zero-shot prompting without conversational demonstrations. This evaluates the simulation of age-associated patterns by already pretrained models; it does not show that those models acquired language through the corresponding developmental stages.

In the human reference, older age groups generally show lower concreteness and content-word density, longer utterances and deeper dependency structures, and lower semantic alignment with greater within-speaker semantic diversity. The models reproduce these directions to different extents. Matching a direction does not imply matching its level or slope: for example, few-shot Llama 3 produces a relatively flat length profile across age groups. The embedding measures describe semantic similarity and dispersion; they do not directly measure how much new information a child contributes.


The comparison between models depends on the metric. Under zero-shot prompting, GPT-4o is closer to the CHILDES reference on concreteness and utterance length, whereas Llama 3 is closer on semantic alignment. The few-shot condition reduces Llama 3's utterance length and syntactic complexity and brings its semantic alignment closer to the human reference, but does not improve every measure. Because the benchmark reports separate metrics without a combined score, these results support a profile of strengths and discrepancies rather than a single overall model ranking.



\paragraph{Caregiver-LLM} In simulating caregiver language, zero-shot GPT-4o more closely approximated CHILDES values for utterance-level measures, while Llama 3 achieved better alignment with word-level and dialogue-level metrics. The few-shot condition brought several properties closer to CHILDES, especially utterance length and semantic diversity, while other properties remained discrepant. For example, GPT-4o's semantic alignment increased further above the human reference. The direction and size of improvement therefore depend on the metric. Notably, both models continued to diverge from human data at the dialogue level, exhibiting higher semantic alignment and lower semantic diversity than actual caregivers in the CHILDES corpus. Overall, there were no clear developmental patterns in CHILDES caregivers' behaviors.
...
To verify this qualitative observation statistically, we ran, for each of the two interactive measures (Dialog\_alignment and Dialog\_diversity), linear regressions comparing models' output to the CHILDES reference, testing all configurations: 2 LLMs x 2 prompting strategies x 2 benchmarking approach. The reported regressions found statistically significant differences in all 16 comparisons (8 for each measure), consistent with the descriptive discrepancies in Figure~\ref{fig:caregiver_multi}. These tests concern the two selected embedding measures and do not establish a general difference in all aspects of interaction.
% AUTHOR CHECK: report the regression formula, observation unit, clustering, effect sizes, intervals, and multiplicity handling before interpreting the precision of these tests.
...
\section{Limitations}
The supplied few-shot templates combine conversational demonstrations with explicit output-length instructions (Section~\ref{sec:prompts}). Differences from zero-shot prompting therefore reflect this combined change, and cannot isolate the effect of examples on length or complexity.
...
A primary constraint concerns our selection of evaluation metrics. We relied on established measures from previous research to assess LLMs at word, sentence, and dialogue levels. For children's language production, these metrics successfully captured developmental trajectories, enabling age-specific benchmarking of LLMs (Figure~\ref{fig:child_single}). However, when applied to caregiver language, these same metrics failed to reveal clear developmental patterns (Figure~\ref{fig:caregiver_single}), thereby limiting the precision of our assessment. This asymmetry highlights the need for developing more refined metrics that can better capture the subtle adaptations in caregiver speech across different stages of child development.
...
\section{Supplementary materials: prompt templates}
\label{sec:prompts}
...
\section{Supplementary materials: fine-tuning details}
...
\section{Supplementary materials: example generations}
