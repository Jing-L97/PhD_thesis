...
\noindent \textbf{Pre-training baselines}  We use a customized version of GPT-2 (2 layers, hidden size 512, 8 attention heads), pre-trained on CHILDES \citep{macwhinney2000childes}, using three pre-training scales (0.1M, 1M, and 10M words; see Section~\ref{sec:rlhf_training}).

\noindent \textbf{Empirical Reward Models} For each feedback type, we trained reward models to capture naturally occurring patterns of caregiver feedback in child--caregiver interactions (using CHILDES dataset). Each reward model is trained to map children's utterances to a scalar, representing the valence of the target feedback (see Section~\ref{sec:rlhf_training}). %Specifically, we fine-tuned a pre-trained DeBERTa model to predict the valence of the target feedback from children’s utterances.
The valence values themselves were annotated prior to reward-model training using a range of automatic annotation tools (see Section~\ref{sec:rlhf_annotation}).

\noindent \textbf{Topline Reward Model}: we use a grammar-targeted reward as a reference condition for what direct grammatical supervision can achieve in this framework. It is trained on labeled BLiMP and Zorro materials (Section~\ref{sec:rlhf_training}), rather than on observed caregiver feedback. It is therefore not a formal upper bound or an independent test of developmental sufficiency.
...
First, we compared the input-only baseline models with the same models fine-tuned using the grammar-targeted reference reward (Figure~\ref{fig:topline} in the supplementary analyses). This condition improves minimal-pair performance and generated grammaticality relative to pretraining alone. Because its reward model is trained on BLiMP and Zorro materials, the benchmark gains demonstrate the effect of direct grammar-targeted supervision within this setup; independent generalization would require documented separation of reward-training and evaluation items.
%However, gains on the Zorro and BLiMP benchmarks remain relatively small compared to generated-utterance grammaticality.

Moving to the empirical reward models, we found no consistent improvement on the minimal-pair benchmarks (Figure~\ref{fig:benchmark_reward_scale} in the supplementary analyses). Across Zorro and BLiMP, none of the reward types reliably improved over the baseline. This echoes a broader pattern in recent studies showing
...
Another concern is that reward fine-tuning may narrow the model’s production distribution, reducing diversity and potentially harming generalization. However, while this may be partly true, some narrowing may still be compatible with grammatical learning, and perhaps even expected if the model moves away from noisy or ill-formed productions. For instance, in our analyses, the grammar topline was relatively the least lexically diverse condition (Figure~\ref{fig:covariates_reward_scale}, Lexical Entropy, supplementary analyses), yet it produced the clearest gains on the minimal-pair benchmarks (Figure~\ref{fig:topline}, supplementary analyses). Thus, reduced lexical diversity did not prevent improvement on these benchmark scores. Because the topline reward used BLiMP and Zorro training materials, this comparison does not independently establish grammatical generalization. The more central question is whether reward-induced changes target grammatical phenomena that can generalize beyond the sampled utterances themselves.
...
\clearpage
\section{Supplementary materials: additional analyses}
\label{app:analyses}
...
\clearpage
\section{Supplementary materials: automatic labeling of rewards}
\label{sec:rlhf_annotation}

We automatically annotated caregiver feedback signals in child-caregiver conversations and used these annotations to supervise reward model training (see Section~\ref{sec:rlhf_training}).
...
\section{Supplementary materials: baseline and reward-model training}
...
For each of the four caregiver feedback dimensions above, we train a reward model that maps a child utterance to the corresponding valence of the caregiver response, as annotated using the procedures described in Section~\ref{sec:rlhf_annotation}.
...
\section{Supplementary materials: RL fine-tuning details}
\label{sec:rlhf_ppo}
